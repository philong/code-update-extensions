...
def parse_age_threshold(age_str):
    ...
    if not match:
        raise ValueError(f"Invalid age format: '{age_str}'. Expected format like '24h', '1d', '30m'.")
...
def check_updates(installed_exts, target_platform, vscode_version=None, exclude_prerelease=True, min_release_age=None):
    ...
    for i in range(0, len(ext_ids), batch_size):
        ...
        if sys.stdout.isatty():
            ...
            bar = '=' * filled_len + ' ' * (bar_len - filled_len)
            sys.stdout.write(f"\r\033[KChecking updates: [{bar}] {percent}% ({current_count}/{len(ext_ids)})")
            sys.stdout.flush()
        ...
        for ext in extensions:
            ...
            try:
                ...
                if parse_version(latest_version) > parse_version(installed_ver):
                    ...
                    latest_release_date = last_updated[:10] if len(last_updated) >= 10 else last_updated
                    ...
                    if eligible_ver_obj:
                        ...
                        if parse_version(el_ver) > parse_version(installed_ver):
                            ...
                            eligible_release_date = el_updated[:10] if len(el_updated) >= 10 else el_updated
                            eligible_platform = eligible_ver_obj.get("targetPlatform") or "universal"
                    ...
                    updates.append({
                        ...
                        "latest_platform": latest_ver_obj.get("targetPlatform") or "universal",
    ...
    for update in updates:
        ...
        if os.path.exists(filepath):
            try:
                ...
                print(f"  {Colors.YELLOW}Warning: Could not remove temporary file {filepath}: {e}{Colors.ENDC}", file=sys.stderr)
...
def install_updates(updates, download_dir, code_binary="code"):
    for update in updates:
        ...
        if not os.path.exists(filepath):
            print(f"{Colors.RED}VSIX file not found for installation: {filepath}{Colors.ENDC}", file=sys.stderr)
            continue
...
def main():
    ...
    if updates:
        if args.yes:
            ...
            print(f"{Colors.BOLD}{'Extension ID':<45} {'Installed':<12} {'Eligible':<12} {'Latest':<12} {'Release Date':<15} {'Platform':<12}{Colors.ENDC}")
            print("-" * 115)
            for update in updates:
                if update["eligible"]:
                ...
                print(f"{Colors.CYAN}{update['id']:<45}{Colors.ENDC} "
